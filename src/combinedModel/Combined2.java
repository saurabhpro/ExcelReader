                    ...
                    case ABSENT:
                        ...
                        for (HrnetDetails hr : hrnetDetails) {
                            ...
                            if (hr.employeeID.equals(bd.empId)) {
                                ...
                                while (leaveTime > 0) {
                                    if (leaveTime == 0.5) {
                                        LocalTime halfWorkTime = LocalTime.of((int) leaveTime * 8, 0);
                                    /*    if(hr.attendanceOfLeave.getLeaveType() != LeaveType.WORK_FROM_HOME)
                                        {
                                            halfWorkTime =  TimeManager.calculateTimeDifference(bd.attendanceOfDate[changeDatesRange].getCheckOut(), bd.attendanceOfDate[changeDatesRange].getCheckIn(), bd.attendanceOfDate[changeDatesRange].getCurrentDate());
                                            if()

                                        }*/
                                        bd.attendanceOfDate[changeDatesRange].setWorkTimeForDay(halfWorkTime);
        ...
        while (biometricDetailsIterator.hasNext()) {
            ...
            if (bd.getNumberOfLeaves() == 0) {
                ...
                while (hrnetDetailsIterator.hasNext()) {
                    ...
                    if (hr.employeeID.equals(bd.empId)) {
                        newHrnetDetails.add(new HrnetDetails(hr.employeeID, hr.name, hr.attendanceOfLeave.getRequestId(), hr.attendanceOfLeave));
                    }
                }
